...
public interface LivreRepository extends JpaRepository<Livre, Long> {
    ...
    @Query(value = "select distinct livre from Livre livre left join fetch livre.themes",
        countQuery = "select count(distinct livre) from Livre livre")
    Page<Livre> findAllWithEagerRelationships(Pageable pageable);

    @Query("select distinct livre from Livre livre left join fetch livre.themes")
    List<Livre> findAllWithEagerRelationships();

    @Query("select livre from Livre livre left join fetch livre.themes where livre.id =:id")
